                ...
                if not key == "ESCAPE":
                    if board[y][x] == 0:
                        ...
                        if not emptyValid(board, key, x, y):
                            pygame.draw.rect(screen, (255, 0, 0) ,(x*newWidth+10, y*newWidth+10, 50, 50), 3)
                        board[y][x] = int(key)
                        printNum(key, x, y) 
        pygame.display.flip()
...
def solve(bo):
    ...
    for i in range(1, 10):
        if valid(bo, i, (row, col)):
            pygame.draw.rect(screen, (101, 196, 100) ,(col*newWidth+10, row*newWidth+10, 50, 50), 3)
...
def drawBoard():    
    ...
    textsurface1 = myfont1.render("Hover over blank space and press number to enter \n Escape to remove", False, (0, 0, 0))
    screen.blit(textsurface1,(25, 610))

    for i in range(1, 9):
        if i % 3 == 0:
            pygame.draw.line(screen, (7, 7, 7), (newWidth*i, 0), (newWidth*i, 600), 3)
            pygame.draw.line(screen, (7, 7, 7), (0, newWidth*i), (600, newWidth*i), 3)
        else:
            pygame.draw.line(screen, (7, 7, 7), (newWidth*i, 0), (newWidth*i, 600), 2)
            pygame.draw.line(screen, (7, 7, 7), (0, newWidth*i), (600, newWidth*i), 2)
        i = i + 1   
...
def printNum (num, x, y):
    ...
    offset = 23
    screen.blit(img, ((newWidth*x)+offset, (newWidth*y)+offset)) #add font to screen
