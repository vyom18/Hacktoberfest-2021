...
a=res.content
try:
    b=a.decode().split('"formats":[{"itag":18,"url":"')#using simple text analysis rather then import havey modules like bs4 for work load
    c=b[1].split('","mimeType":"video/mp4;')[0]
    #unquote(c)  
    url=eval(f"unquote('{c}')")#for adding more threads print this url and you can see bytes=1455-s5454 some thing like this and        #                           maupulate the thread range accoring to the thread for more faster experiecnce
    finished=False
except:
...
t1=Thread(target=download,args=(url,))#You can also use thread pools of you want to if you are going to download many video at once
