...
def cross(state, path):
    ...
    if state[2] == 'L':
        ...
        if state[0] >= 1 and state[1] >=1:
            cross([state[0]-1,state[1]-1,'R'], path[:] + ['One cannible and one missionary go right'])
        if state[1] >= 1:
            cross([state[0],state[1]-1,'R'], path[:] + ['One missionaries goes right'])
        if state[1] >= 2:
            cross([state[0],state[1]-2,'R'], path[:] + ['Two missionaries go right'])
    ...
    if state[2] == 'R':
        ...
        if C-state[0] >= 1 and M-state[1] >=1:
            cross([state[0]+1,state[1]+1,'L'], path[:] + ['One cannible and one missionary go left'])
        if M-state[1] >= 1:
